...
class ShulkerBoxInventory extends ContainerInventory{
	...
	public function onClose(Player $who) : void{
		//var_dump($who->getLevelNonNull()->getId());
		if(count($this->getViewers()) >= 1 && ($level = $who->getLevel()) instanceof Level){
			$this->broadcastBlockEventPacket(false);
			$level->broadcastLevelSoundEvent(/*$this->getHolder()->add(0.5, 0.5, 0.5)*/ $who->getPosition(), LevelSoundEventPacket::SOUND_SHULKERBOX_CLOSED);
		}
		parent::onClose($who);
	}
	...
	public function onOpen(Player $who) : void{
		...
		if(count($this->getViewers()) >= 1 && ($level = $who->getLevel()) instanceof Level){
			...
			$level->broadcastLevelSoundEvent(/*$this->getHolder()->add(0.5, 0.5, 0.5)*/ $who->getPosition(), LevelSoundEventPacket::SOUND_SHULKERBOX_OPEN);
		}
	}
