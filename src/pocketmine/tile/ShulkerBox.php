...
class ShulkerBox extends Spawnable implements InventoryHolder, Container, Nameable{
	...
	public function close() : void{
		if(!$this->closed){
			...
			//$pos = $this->asPosition();
			//$level = $pos->getLevel();
			//$players = $level->getChunkPlayers($pos->getFloorX() >> 4, $pos->getFloorZ() >> 4);
			//var_dump($this->asPosition());
			//$this->getLevel()->broadcastLevelSoundEvent($this, LevelSoundEventPacket::SOUND_SHULKERBOX_CLOSED);
			//$pk = LevelSoundEventPacket::create(
			//	LevelSoundEventPacket::SOUND_SHULKERBOX_CLOSED,
			//	$pos->asVector3()
			//);
